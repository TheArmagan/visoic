/*{
  "DESCRIPTION": "Maps brightness to depth, displacing pixels along the Z-axis to form a point cloud with adjustable density and extrusion.",
  "CREDIT": "spleennooname",
  "CATEGORIES": [
    "3D", "Texture"
  ],
  "INPUTS": [
...
{
	return texture2D( inputImage, .5+(vec2(x))/RENDERSIZE.x*SAMPLERADIUS, RENDERSIZE.y);
}
...
vec4 Rand( in int x )
{
	...
	return texture2D( inputImage, uv, -10.0 );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float offset = 0.25;//iGlobalTime*.01;	
