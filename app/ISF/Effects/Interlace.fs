/*{
    ...
    "DESCRIPTION": "Alternates row visibility between the current and previous frame based on an adjustable line size threshold.",
    "INPUTS": [
        {
            "NAME": "inputImage",
            "TYPE": "image"
...
void main()
{
	...
	//	else if this is the second pass- read the position value from the buffer which stores it
	else if (PASSINDEX == 1)	{
		vec4		lastRowVal = IMG_PIXEL(lastRow,vec2(0.5));
		vec2		pixelCoord = isf_FragNormCoord * RENDERSIZE;
		
		if (mod(floor(pixelCoord.y),2.0 * lineSize) < lineSize + lineSize * lastRowVal.x)
			gl_FragColor = IMG_NORM_PIXEL(inputImage,isf_FragNormCoord);
		else
			gl_FragColor = IMG_NORM_PIXEL(lastFrame,isf_FragNormCoord);
	}
	//	else this is the final pass- just output the last frame
	else	{
		gl_FragColor = IMG_NORM_PIXEL(lastFrame, isf_FragNormCoord);
	}
}
