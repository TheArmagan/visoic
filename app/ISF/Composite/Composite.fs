/*
{
  ...
  "INPUTS": [
    ...
    },
    {
      ...
      "LABELS": ["Normal", "Add", "Subtract", "Multiply", "Darken", "Color Burn", "Linear Burn"],
      "DEFAULT": 0
    },
    {
      "NAME": "blendModeB",
...
void main() {
  ...
  if (blendModeProgress <= 0.0) {
    ...
    blendedColor = blend(layerColor, baseColor, blendModeA, 1.0);
  } 
  else if (blendModeProgress >= 1.0) {
    // Only use blend mode B
    blendedColor = blend(layerColor, baseColor, blendModeB, 1.0);
  } 
  else {
    // Mix between the two blend modes using the phase
    vec4 blendedColorA = blend(layerColor, baseColor, blendModeA, 1.0);
    vec4 blendedColorB = blend(layerColor, baseColor, blendModeB, 1.0);
